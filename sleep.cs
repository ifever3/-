    ...
    public partial class sleep : Form
    {
        ...
        private void setDataGridView()
        {
           SqlConnection sqlconn = new SqlConnection("server=LAPTOP-LJQH2OK2;uid=sa;pwd=123;database=健身运动");
            sqlconn.Open();
            string sql = string.Format("select * from sleep where 用户名='{0}'",acc.Username);//构造一个SQL语句字符串   
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            SqlConnection sqlconn = new SqlConnection("server=LAPTOP-LJQH2OK2;uid=sa;pwd=123;database=健身运动");
            sqlconn.Open();
            string sql = string.Format("select * from sleep where 用户名='{0}'and 日期='{1}'", acc.Username, dateTimePicker1.Value.ToString("yyyy-MM-dd"));//构造一个SQL语句字符串   
            SqlDataAdapter sda = new SqlDataAdapter(sql, sqlconn);
            DataSet ds = new DataSet();//用SqlDataAdapter来获得数据库中的数据，填充至DataSet中
            sda.Fill(ds);//填充表
                    ...
                    else if (dataGridView1.Columns[e.ColumnIndex].HeaderText == "Delete")
                    {
                        ...
                        if (dr == DialogResult.Yes)
                        {
                            ...
                            try
                            {
                                ...
                                sqlconn.Open();
                                string sql = string.Format("delete from sleep where num={0}", Main.sid);
